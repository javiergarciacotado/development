...
public class Lesson3 {
  ...
  static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
    ...
    if (parallel) {
      IntStream.range(0, LIST_SIZE)
              .parallel()
              .forEach(i -> IntStream.range(0, i).parallel()
                .forEach(j -> calculateDistance(wordList, distances, i, j))
              );
  ...
  static void calculateDistance(List<String> wordList, int[][] distances, int i, int j) {
  ...
  static List<String> processWords(List<String> wordList, boolean parallel) {
    ...
    return wordListStream.sorted((s1, s2) -> s1.length() - s2.length())
